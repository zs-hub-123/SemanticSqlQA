# SemanticSqlQA - 企业级Text2SQL智能问答平台 V5.2

<p align="center">
  <strong>四层架构 · MQL中间层 · 语义层屏蔽 · 强约束SQL生成</strong>
</p>
...
- ✅ **MQL中间层**（结构化指标查询语言，语义解析与SQL生成解耦）
- ✅ **语义层屏蔽物理表差异**（6张核心配置表）
- ✅ **增强指标元数据**（非累加标识、统计周期、版本管理）
- ✅ **业务规则引擎**（口径规则、过滤条件自动应用）
- ✅ **血缘追溯**（返回完整口径说明和数据来源）
- ✅ **SQL白名单校验**（防止大模型幻觉编造表/字段）
- ✅ **递归关联路径推导**（只存直接关系，自动推导多层，最大3层深度）
- ✅ **流式输出支持**（SSE实时返回）
- ✅ **多模型切换**（支持配置多个大模型，动态切换）

### V5.2 优化亮点

- 🚀 **移除Milvus向量库**（更轻量、更快、无外部依赖）
- 🛡️ **MQL强控制SQL生成**（业务规则强制注入提示词）
- ⚠️ **非累加指标跨粒度拦截**（防止UV等去重类指标错误聚合）
- 🔒 **表关联路径限制3层**（防止性能爆炸和无效JOIN）
...
│  ├─────────────────────────────────────────────────┤ │
│  │ business_rules 业务规则表 (V5.0新增)            │ │
│  │ dimension_hierarchies 维度层级表 (V5.0新增)     │ │
│  └─────────────────────────────────────────────────┘ │
│       ↓ 候选表筛选 + 关系查询(最大3层) + 规则应用      │
└──────────────────────┬──────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────┐
│             Layer4：数据服务层                       │
